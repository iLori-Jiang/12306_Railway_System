create function ticket_remain(travelid integer, seat character varying, depart integer, arrive integer)
    returns TABLE(seatremain integer, totalprice numeric)
    language plpgsql
as
$$
declare
...
begin
    ...
    return query
    select min(seat_left), sum(price) from (
    select * from (
    select sr.travel_id, sr.seat_type, sr.seat_left, sr.depart_station_id, sr.arrive_station_id, ps2.depart_time, ps.arrive_time,sr.price from seat_remain sr
    ...
    and arrive_time <= arrivetime) sub2;
